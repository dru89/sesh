...
- Stop losing Claude Code CLI sessions whose transcript has been cleaned up. Claude Code prunes `~/.claude/projects` while `history.jsonl` keeps every prompt, so those sessions read back with no text at all — no summary could be generated for them and `sesh show` displayed nothing. They now fall back to the prompts still on disk. On the author's machine that was every `claude-code` session
