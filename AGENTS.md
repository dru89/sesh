...
│   ├── jsonl.go              # JSONL line/decode iterators: open, close, scan-buffer sizing, scan-error reporting
...
- **One scan-buffer size for every JSONL reader.** Agent history and transcripts are JSONL, so one line holds one message — and a pasted file or a long tool result makes that line megabytes wide. `bufio.Scanner` does not skip an oversized line and carry on: `Scan()` returns false and the read is finished, so a cap set too low drops the whole rest of the file. The original caps (1 MB, and 256 KB in `extractSlug`) were unmeasured, and the largest line in a 252-file sample of `~/.claude/projects` was 1484 KB — past all of them, with the offending line at 320 of 340, so titles, session text, and summary input lost the newest 20 messages of that transcript with no warning. All readers now go through `provider.jsonlLines()` (64 KB start, 32 MB ceiling; the buffer grows on demand, so the ceiling costs nothing on normal files), a range-over-func iterator that owns the open, the close, the buffer sizing, and the `Err()` report, so a new reader gets all four without asking. `jsonlLinesFrom()` is the variant for a caller that opened the file itself, and `jsonlDecode[T]()` composes over either to skip lines that do not parse as T. `countHumanTurns` deliberately stays on the line layer: its `bytes.Contains` prefilter in front of the unmarshal measured 2.3x faster than decoding every line over this machine's Cowork corpus, because the record pulls `Content` out as a `json.RawMessage` and a decode-everything pass copies the body of every assistant message to reach fields only user records have. Keeping it in one place matters because `claude.go` and `claude_cowork.go` read the same files — divergent caps mean one reader sees a session the other truncates. `countHumanTurns` in `claude_cowork.go` shows why the shared limit is not only about session text: it decides whether a scheduled run was engaged with, so a short read undercounts turns and folds a session the user worked in into a collapsed group. Every scan site also reports `Err()` through `warnScanErr()`; callers keep their partial data, but the truncation is no longer silent.
- **A shape change on the producer side is detected in aggregate, not per file.** `encoding/json` does not fail on one: unknown fields are ignored and missing ones zero-filled, so if an agent renames or moves a field, every line still decodes cleanly into an empty record and each reader's role/id test quietly rejects it. `jsonlDecode`'s skip-on-error never fires, because there is no error. Nothing downstream notices either, and the reason is uncomfortable: empty session text is *deliberately* skipped by `runIndex` and `lazyIndex` (summarizing a bare title produces "I don't have access to this session"), which leaves `Health.RecordRun` at `attempted == 0`, and `RecordRun` ignores empty runs on purpose. The one mechanism built to report broken summarization is, for this failure, built not to fire — while the picker keeps advising `sesh index`, the command that just skipped everything.

  `Health.RecordNoUsableText` closes it at that exact spot. Both index paths already compute how many sessions they skipped for want of text; they now pass that count and one example session ID instead of discarding it, and the picker hint gains a middle tier: broken generation > unreadable text > not-yet-indexed. Any run that attempts even one summary clears the state, since attempting proves text was readable.

  Detecting this in the readers was tried first and rejected. It needed three touch points per reader (wrap the iterator, increment on a recognized record, report at the end), which is the same forget-and-it-silently-stops-working property that produced the oversized-buffer bug — a guard whose failure mode is the bug class it guards. Worse, it was silent exactly where it mattered: transcripts are read in `lazyIndex`, under the alt screen, so the warning went nowhere the user could see. The aggregate check costs no reader anything and is provider-agnostic, so it covers an OpenCode schema change too. What it gives up is naming the offending file, which the example session ID mostly recovers.

  `ListSessions` is the one per-source check, because `history.jsonl` is that provider's entire input and an unreadable one has to surface as an error rather than an empty list. It reuses the `os.Stat` already done for the existence check: bytes on disk with no entry carrying a `sessionId` means the format moved. Measured before trusting it — all 729 lines of the real `history.jsonl` carry one.
...
- `summary/health.go` — JSON record at `~/.cache/sesh/index-health.json` tracking whether generation actually works. `RecordRun(attempted, succeeded, firstErr, command)` increments a counter when every attempted summary in a run failed and resets it on any success; `Failing()` reports once the counter reaches `FailureHintThreshold` (3). Empty runs are ignored so "nothing to summarize" is not read as evidence either way — except when the run was empty *because* every session read back textless, which `RecordNoUsableText` records separately and `NoUsableText()` reports past `NoTextHintThreshold` (5).
